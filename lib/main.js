...
class MapboxCircle {
     ...
     * @public
     */
    addTo(map, beforeLayer) {
        const addCircleAssetsOnMap = () => {
            map.addSource(this._circleSourceId, this._getMapSourceGeoJSON());

            map.addLayer(this._getCircleStrokeLayer(), beforeLayer);
            map.addLayer(this._getCircleFillLayer(), beforeLayer);
            this._bindCircleFillListeners(map);
            map.on('zoomend', this._onZoomEnd);

            if (this.options.editable) {
                map.addLayer(this._getCircleCenterHandleLayer(), beforeLayer);
                this._bindCenterHandleListeners(map);

                map.addLayer(this._getCircleRadiusHandlesLayer(), beforeLayer);
