...
class MapboxCircle {
     ...
     * @public
     */
    addTo(map, behindLayer) {
        const addCircleAssetsOnMap = () => {
            map.addSource(this._circleSourceId, this._getMapSourceGeoJSON());

            map.addLayer(this._getCircleStrokeLayer(), behindLayer);
            map.addLayer(this._getCircleFillLayer(), behindLayer);
            this._bindCircleFillListeners(map);
            map.on('zoomend', this._onZoomEnd);

            if (this.options.editable) {
                map.addLayer(this._getCircleCenterHandleLayer(), behindLayer);
                this._bindCenterHandleListeners(map);

                map.addLayer(this._getCircleRadiusHandlesLayer(), behindLayer);
